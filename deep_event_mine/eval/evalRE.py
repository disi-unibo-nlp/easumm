...
from deep_event_mine.utils.utils import write_lines
...
class SelectClass:
    ...
    def __call__(self, *inputs):
        ...
        # negative prediction means no role
        ...
        # it can happen that with obtain a label for the relation from one direction and the opposite
        # label for the opposite direction. In this case we keep the first direction label
        ...
        # return the correct label among all the possible cases
...
def get_entities_info_input(eval_data, eval_data_ids, ner_out, nn_span_indices):
    ...
    ent_ann = {'span_indices': nn_span_indices, 'ner_preds': ner_out['preds'], 'words': words,
               ...
               'ner_terms': ner_terms, 'fids': fids}

    return ent_ann


def get_entities_info(ent_ann, params):
