...
def get_entity_attrs_wordpos(e_idx, words, offset, span_indices, sub_to_words, word_pos):
    e_span_indice = span_indices[e_idx]
    e_words = []
    e_wpos = []
    e_offset = [-1, -1]
    ...
    for idx in range(e_span_indice[0], e_span_indice[1] + 1):
        if sub_to_words[idx] != curr_word_idx:
            e_words.append(words[sub_to_words[idx]])
            e_wpos.append(word_pos[sub_to_words[idx]])
            curr_word_idx = sub_to_words[idx]
        if idx == e_span_indice[0]:
            e_offset[0] = offset[sub_to_words[idx]][0]
        if idx == e_span_indice[1]:
    ...
    return ' '.join(e_words), (e_offset[0], e_offset[1]), e_wpos


# generate entity
def generate_entities_wpos(fids, all_e_preds, all_words, all_offsets, all_span_terms, all_span_indices,
                           all_sub_to_words,
                           params):
    ...
    word_num = 0
    words_pos = []
    for fid_, words_ in zip(fids, all_words):
        words_pos_ = []
        for fid, words in zip(fid_, words_):
            word_pos = []
            for _ in words:
                if fid != last_fid:
                    word_num = 0
                    word_pos.append(word_num)
                else:
                    word_pos.append(word_num)
            ...
            words_pos_.append(word_pos)

        words_pos.append(words_pos_)

    for xi, (words_pos_, fids_, e_preds_, words_, offset_, span_indices_, sub_to_words_) in enumerate(
            zip(words_pos, fids, all_e_preds, all_words, all_offsets, all_span_indices, all_sub_to_words)):

        for xb, (word_pos, fid, e_preds, words, offset, span_indices, sub_to_words) in enumerate(
                zip(words_pos_, fids_, e_preds_, words_, offset_, span_indices_, sub_to_words_)):
            ...
            for xx, pred in enumerate(e_preds):
                if pred > 0:
                    ...
                    e_words, e_offset, e_wpos = get_entity_attrs_wordpos(xx, words, offset, span_indices, sub_to_words,
                                                                         word_pos)
                    preds[(xi, (xb, xx))] = [e_term, e_type_id, e_offset, e_words, e_wpos]
...
def write_ev_2file(pred_output, pred_ents, result_dir, g_entity_ids_, params):
    ...
    for fid, preds in pred_output.items():
        ...
        enid_mapping, en_preds_out_, a2_ents_ = mapping_entity_id(pred_ents[fid], g_entity_ids_[fid], rev_type_map,
                                                                  params)
...
def write_files_wpos(pred_output, pred_ents, result_dir, g_entity_ids_, params):
    a2dir = result_dir + 'ev-tok-a2/'
    anndir = result_dir + 'ev-tok-ann/'
    rev_type_map = params['mappings']['rev_type_map']
    ...
    for fid, preds in pred_output.items():
        ...
        for pr_id, e_pred in pred_ents[fid].items():
            ...
            w_pos = ' '.join(str(e) for e in e_pred[4])
            output = ''.join(
                [e_id, '\t', rev_type_map[e_pred[1]], ' ', str(e_pred[2][0]), ' ', str(e_pred[2][1]), '\t',
                 e_pred[3], '\t', w_pos, '\n'])
        ...
        # write a2 files
        a2_fid_dir = a2dir + fid + '.a2'
        with open(a2_fid_dir, 'w') as o2file:

            if os.path.exists(a2_fid_dir):
                o2file.truncate()
        ...
        # write ann file
        ann_a1_dir = anndir + fid + '.a1'
        with open(ann_a1_dir, 'w') as o1file:

            if os.path.exists(ann_a1_dir):
                o1file.truncate()

            for entity in ann_en_lines:
                o1file.write(entity)

        ann_a2_dir = anndir + fid + '.a2'
        with open(ann_a2_dir, 'w') as annfile:

            if os.path.exists(ann_a2_dir):
                annfile.truncate()
...
# generate event output and evaluation
def write_events_wpos(fids, all_ent_preds, all_words, all_offsets, all_span_terms, all_span_indices, all_sub_to_words,
                      all_ev_preds, g_entity_ids_, params, result_dir):

    # generate predicted entities
    pred_ents = generate_entities_wpos(fids=fids,
                                       all_e_preds=all_ent_preds,
                                       all_words=all_words,
                                       all_offsets=all_offsets,
                                       all_span_terms=all_span_terms,
    ...
    write_files_wpos(preds_output, pred_ents, result_dir, g_entity_ids_, params)
