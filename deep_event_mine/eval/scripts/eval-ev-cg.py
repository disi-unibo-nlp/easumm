...
def print_subset_total(eventset, totalstr, delta, authors_scores):
    ...
    if delta:
        total_type_match_answer_auth = authors_scores['match_answer']
        total_type_match_gold_auth = authors_scores['match_gold']
        total_type_over_match_auth = authors_scores['over_match']
        total_type_partial_match_auth = authors_scores['partial_match']
        total_type_false_positive_auth = authors_scores['false_positive']
        total_type_false_negative_auth = authors_scores['false_negative']
    ...
    if delta:
        subtotal_match_answer_auth, subtotal_match_gold_auth = 0, 0
        subtotal_partial_match_auth, subtotal_over_match_auth = 0, 0
        subtotal_false_positive_auth, subtotal_false_negative_auth = 0, 0

        for t in eventset:

            subtotal_match_answer_auth += total_type_match_answer_auth[t]
            subtotal_match_gold_auth += total_type_match_gold_auth[t]
            subtotal_over_match_auth += total_type_over_match_auth[t]
            subtotal_partial_match_auth += total_type_partial_match_auth[t]
            subtotal_false_positive_auth += total_type_false_positive_auth[t]
            subtotal_false_negative_auth += total_type_false_negative_auth[t]

        precision_auth, recall_auth, F_auth = prec_rec_F(subtotal_match_answer_auth, subtotal_match_gold_auth + subtotal_over_match_auth,
                                      subtotal_false_positive_auth, subtotal_false_negative_auth)

        precision = precision - precision_auth
        recall = recall - recall_auth
        F = F - F_auth
...
# json_scores = {'match_answer': total_type_match_answer, 'match_gold': total_type_match_gold,
#                'over_match': total_type_over_match, 'false_positive': total_type_false_positive,
#                'false_negative': total_type_false_negative, 'partial_match': total_type_partial_match}
#
# with open('model_scores/authors_scores.txt', 'w') as outfile:
#     json.dump(json_scores, outfile)


if delta:
    with open('model_scores/authors_scores.txt') as json_file:
        authors_scores = json.load(json_file)
...
for i, t in enumerate(output_event_type_order):
    if "TOTAL" in t:
        # special case: print out a subset total in this position
        eventset = subtotal_event_set[t]
        print_subset_total(eventset, t, delta, authors_scores)
    else:
        ...
        if delta:
            precision_auth, recall_auth, F_auth = prec_rec_F(total_type_match_answer_auth[t],
                                              total_type_match_gold_auth[t] + total_type_over_match_auth[t],
                                              total_type_false_positive_auth[t], total_type_false_negative_auth[t])
