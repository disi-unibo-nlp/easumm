...
def main(preddir, refdir):
    global options

    class Options:
        def __init__(self, preddir, refdir):
            self.preddir = preddir
            self.refdir = refdir
            self.spliteventeval = False
            self.softboundary = True
            self.partialrecursive = True
            self.singlepenalty = False
            self.verbose = False
            self.outputdir = None
            self.verifytext = False

    options = Options(preddir, refdir)
    ...
    precision, recall, F = compute_scores(eventset)
