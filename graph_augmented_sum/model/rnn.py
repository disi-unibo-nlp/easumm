...
def lstm_multiembedding_encoder(sequence, lstm,
    ...
    We reorder the documents to the largest to the smaller for speeding up
    computations
    """

    # number of documents e.g. 32
    batch_size = sequence.size(0)
    if not lstm.batch_first:
        # e.g. from (32, 775, 768) to (775, 32, 768)
    ...
    if seq_lens:
        ...
        # sort documents from the largest to the smalles
        # sort_ind is of length 32 e.g. [2, 0, 1, .....]
        sort_ind = sorted(range(len(seq_lens)),
                          key=lambda i: seq_lens[i], reverse=True)
        seq_lens = [seq_lens[i] for i in sort_ind]
        # e.g. (775, 32, 768) the first document is largest and so on
    ...
    if seq_lens:
        ...
        # packed_seq.data: contains all the embeddings e.g. (2151, 768) where 2151 = sum(seq_lens)
        # packed_seq.batch_sizes: batch size for the i-th token e.g (775,) [32, 32,......,1]
